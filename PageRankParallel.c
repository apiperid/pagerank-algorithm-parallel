...
double convergence();
void sortingResult();
double calc_SUM1(int i,int end);
double calc_SUM2(int i,int start,int end);
...
int main(int argc,char **argv)
{
  ...
  printf("PageRank CilkPlus Time = %0.8f\n", seq_time);
...
/*
  ...
  the calculation of the 2 SUMS can be in parallel using the
  commands cilk_spawn and cilk_sync
  however the program will be slower than it is now
...
void gaussSeidelPagerank()
{
  ...
  for(int loops = 0 ;loops<ITERATIONS;loops++)
  {
    ...
    for(int i=0;i<nodes;i++)
    {
      ...
      calc_SUM1(i,i-1);
      calc_SUM2(i,i+1,nodes);
      /* you can use the following 3 commands instead of the above 2:
         NOTE!!! the result will be the same .. but the program will be slower

      cilk_spawn calc_SUM1(i,i-1);
      calc_SUM2(i,i+1,nodes);
      cilk_sync;
     
      */
      pageranksCurrent[i] = (b[i] - SUM1 - SUM2)/(double)(A[i][i]);
    }   
...
/*
  ...
  calculates the SUM of the updated values X(k+1)
...
/*
  ...
  calculates the SUM of the not updated values X(k)
...
/*
  ...
  we make the calculation I-a*P in parallel
  each thread takes a row of the matrix
...
void create_Right_Part_Of_Equation()
{
  ...
  /*  
    ...
    the results will be the same but the program will be slower.
...
/*
  ...
  this can be in easily in parallel

  each thread is responsible for one column of the matrix
  it calculates its sum and then it divides each element of the column with its sum.
...
void createOutput()
{
  ...
  FILE *output = fopen("outputCilkPlus.txt","w");
  if (output == NULL )
  {
    printf("Error opening the files : outputCilkPlus.txt");
    exit(1);
  }
  for(int i=0;i<nodes;i++)
    fprintf(output,"%d\n",nodes_vector[i]);
